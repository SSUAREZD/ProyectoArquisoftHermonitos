...
# Decoradores de caché (usa el backend configurado en settings -> Redis)
...
# -----------------------------
# Cacheamos 60s porque es consultado frecuentemente y no necesita estar al segundo.
...
def bodegas_data_api(request):
    ...
    {
      id_bodega: {
        latitud, longitud, nombre, direccion, capacidad,
        total_disponible, total_reservado, ocupacion_pct
      }
    }
    ocupacion_pct = ((disponible + reservado) / capacidad) * 100 si capacidad > 0.
    Soporta filtro ?bodega_id=
    """
    ...
    for b in qs:
        ...
        ocupacion_pct = ((disp + res) / capacidad) * 100.0 if capacidad > 0 else 0.0
...
# KPIs suelen cambiar con menos frecuencia 
@cache_control(public=True, max_age=120)
@cache_page(120)
def kpis_api(request):
    """KPIs globales o filtrados por ?bodega_id="""
    bodega_id = request.GET.get('bodega_id')
    bodega = Bodega.objects.filter(id=bodega_id).first() if bodega_id else None
    ...
    if bodega:
        ...
        total_capacity = float(bodega.capacidad or 1)
    else:
        ...
        total_disponible = inv_qs.aggregate(total=Sum('cantidad_disponible'))['total'] or 0
        total_reservado = inv_qs.aggregate(total=Sum('cantidad_reservada'))['total'] or 0
        total_capacity = float(Bodega.objects.aggregate(cap=Sum('capacidad'))['cap'] or 1)

    occupancy = round(((total_disponible + total_reservado) / total_capacity) * 100, 2)
    return JsonResponse({
...
# Mezcla disponible vs reservado; 120s para evitar recalcular todo el set cada request
@cache_control(public=True, max_age=120)
@cache_page(120)
...
# Aging cambia lento; 300s reduce costo de cálculo
...
# Top SKUs también puede ir a 300s
@cache_control(public=True, max_age=300)
@cache_page(300)
...
# Tareas por estado se puede cachear 120s
@cache_control(public=True, max_age=120)
@cache_page(120)
